...
Private Sub cmdHapus_Click()
    ...
    If sp_Tindakan("D") = False Then Exit Sub
...
Private Sub subLoadGridSource()
    On Error GoTo errLoad
    Set rs = Nothing
    strSQL = "select * from V_TujuanNRencanaTindakan WHERE DiagnosaKeperawatan LIKE '%" & txtParameter.Text & "%'"
    ...
    With dgTindakan
        ...
        .Columns("PenyebabAsKep").Width = 0
        .Columns("DetailAsKep").Width = 1000
        .Columns("MemoAsKep").Width = 4000
        .Columns("StatusEnabled").Width = 0
        .Columns("NamaAskep").Width = 0
    End With
    Set rs = Nothing
    If dgTindakan.ApproxCount = 0 Then Exit Sub
...
Private Sub txtDiagnosaKeperawatan_Change()
    ...
    strFilterDiagnosa = "WHERE DiagnosaKeperawatan like '%" & txtDiagnosaKeperawatan.Text & "%' or KdDiagnosaKeperawatan like '%" & txtDiagnosaKeperawatan.Text & "%'"
...
Private Sub subLoadDiagnosa()
    On Error GoTo errLoad
    Set rs = Nothing
    strSQL = "select KdDiagnosaKeperawatan, DiagnosaKeperawatan from DiagnosaKeperawatan " & strFilterDiagnosa
...
Private Sub txtDiagnosaKeperawatan_KeyPress(KeyAscii As Integer)
    ...
    If KeyAscii = 13 Then
        ...
        Else
            dcDetailAsKep.SetFocus
            Set rs = Nothing
            strSQL = "select KdDiagnosaKeperawatan from DiagnosaKeperawatan where DiagnosaKeperawatan='" & txtDiagnosaKeperawatan.Text & "'" & strFilterDiagnosa
            rs.Open strSQL, dbConn, adOpenForwardOnly, adLockReadOnly
...
Private Function sp_Tindakan(f_Status As String) As Boolean
    ...
    With dbcmd
        ...
        .CommandText = "AUD_TujuanNRencanaTindakan"
