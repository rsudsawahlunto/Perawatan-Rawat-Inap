...
Private Sub cmdSimpan_Click()
    ...
    If (fgData.TextMatrix(1, 0) = "") And (fgData.TextMatrix(1, 1) = "") And (fgData.TextMatrix(1, 7) = "") Then
        MsgBox "Detail Tindakan Medis masih kosong", vbCritical, "Validation"
        fgData.SetFocus
        Exit Sub
    End If
    With fgData
        For i = 1 To .Rows - 1
            'If (.TextMatrix(i, 0) = "") Or (.TextMatrix(i, 1) = "") Or (.TextMatrix(i, 7) = "") Then 'Jika Ruang operasi, ini dimunculkan
    ...
    With fgData
        For i = 1 To .Rows - 1
            If (.TextMatrix(i, 0) <> "") Or (.TextMatrix(i, 1) <> "") Or (.TextMatrix(i, 7) <> "") Then
                If SP_AUDDetailHasilTindakanMedisPasien(.TextMatrix(i, 5), .TextMatrix(i, 6), IIf(.TextMatrix(i, 7) = "", 0, .TextMatrix(i, 7)), .TextMatrix(i, 3), .TextMatrix(i, 4), "A") = False Then Exit Sub
            End If
        Next i
    End With
...
Private Function SP_AUDDetailHasilTindakanMedisPasien(f_KdPelayananRS As String, f_KdTindakanMedis As String, f_KdKualitasHasil As Integer, f_HasilPeriksa As String, f_MemoHasilPeriksa As String, f_status As String) As Boolean
    ...
    With dbcmd
        ...
        .Parameters.Append .CreateParameter("KdKualitasHasil", adTinyInt, adParamInput, , IIf(Trim(f_KdKualitasHasil) = 0, Null, CInt(Trim(f_KdKualitasHasil))))
...
Private Function SP_AUDHasilTindakanMedis(f_status As String) As Boolean
    ...
    With dbcmd
        ...
        .Parameters.Append .CreateParameter("KdKeadaanLahirBayi", adVarChar, adParamInput, 2, Null)
...
'    fgData.TextMatrix(fgData.Row, 1) = ""
'    fgData.TextMatrix(fgData.Row, 2) = ""
...
Private Sub dcTindakanOperasi_GotFocus()
    ...
    'Call msubDcSource(dcTindakanOperasi, rs, "SELECT KdTindakanMedis,TindakanMedis From TindakanMedis WHERE KdJenisTindakanMedis LIKE '%" & dcJnsTindakanMedis.BoundText & "%' and KdInstalasi='" & mstrKdInstalasiLogin & "' AND StatusEnabled=1 ")
    'Call msubDcSource(dcTindakanOperasi, rs, "SELECT Distinct KdTindakanMedis,TindakanMedis From V_JenisTindakanBersalinx WHERE KdJenisTindakanMedis = '" & dcJnsTindakanMedis.BoundText & "' and KdRuangan='" & mstrKdRuangan & "' ")
End Sub
...
'        strSQL = "Select KdTindakanMedis,TindakanMedis From TindakanMedis Where (TindakanMedis LIKE '%" & dcTindakanOperasi.Text & "%') And KdInstalasi='" & mstrKdInstalasiLogin & "' And StatusEnabled=1"
...
Sub subLoadDcSource()
    ...
    'Call msubDcSource(dcPelayananRS, rs, "Select KdPelayananRS,NamaPelayanan From V_BiayaPelayananTindakan Where NoPendaftaran='" & mstrNoPen & "' AND NoLab_Rad='" & mstrNoIBS & "' ORDER BY TglPelayanan")
    'Call msubDcSource(dcPelayananRS, rs, "Select KdPelayananRS,NamaPelayanan From V_BiayaPelayananTindakan Where NoPendaftaran='" & mstrNoPen & "' ORDER BY TglPelayanan")
    Call msubDcSource(dcPelayananRS, rs, "Select Distinct KdPelayananRS,NamaPelayanan From V_BiayaPelayananTindakan Where NoPendaftaran='" & mstrNoPen & "'")
    'Call msubDcSource(dcPelayananRS, rs, "Select KdPelayananRS,NamaPelayanan From V_PelayananForRuangan Where KdRuangan = '" & mstrKdRuangan & "' ")
