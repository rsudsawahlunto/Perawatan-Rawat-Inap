...
Begin VB.Form frmClosingDataPelayananTM_OA_Apotik 
   ...
   Caption         =   "Medifist2000 - Data Pelayanan TM,OA,Apotik"
   ClientHeight    =   8280
   ClientLeft      =   45
   ClientTop       =   330
   ClientWidth     =   14430
...
Private Sub subLoadDcSource()
    ...
    If optTindakanMedis.Value = True Then
        Call msubDcSource(dcJenisItem, rs, "SELECT KdJnsPelayanan, Deskripsi FROM JenisPelayanan where StatusEnabled='1' order by Deskripsi")
        Call msubDcSource(dcNamaItem, rs, "SELECT KdPelayananRS, NamaPelayanan FROM ListPelayananRS where StatusEnabled='1' order by NamaPelayanan")
    Else
        Call msubDcSource(dcJenisItem, rs, "SELECT KdDetailJenisBarang, DetailJenisBarang FROM DetailJenisBarang where StatusEnabled='1' order by DetailJenisBarang")
        Call msubDcSource(dcNamaItem, rs, "SELECT KdBarang, NamaBarang From MasterBarang where StatusEnabled='1' order by NamaBarang")
    End If

    Call msubDcSource(dcAsalPasien, rs, "SELECT KdRuangan, NamaRuangan FROM Ruangan")
    Exit Sub
hell:
    Call msubPesanError
End Sub
...
Public Sub subLoadGridSource()
    ...
    If optTindakanMedis.Value = True Then
        frmClosingDataPelayananTM_OA_Apotik.Caption = "Closing Data Pelayanan Tindakan Medis"
        ...
        " AsalPasien , NoClosing, KdItem,  KdAsal, NoLab_Rad FROM V_DaftarDataPelayananTMForClosing" & _
    ...
    ElseIf optObatAlKes.Value = True Then
        frmClosingDataPelayananTM_OA_Apotik.Caption = "Closing Data Pelayanan Obat AlKes"
        ...
        " Administrasi , TotalBiaya, Ruangan, AsalPasien, NoClosing,KdItem,NoLab_Rad,  KdAsal  " & _
        " FROM V_DaftarDataPelayananOAForClosing" & _
        " WHERE TglPelayanan BETWEEN '" & Format(dtpAwal.Value, "yyyy/MM/dd hh:mm:00") & "' AND '" & Format(dtpAkhir.Value, "yyyy/MM/dd hh:mm:59") & "' " & _
        " AND JenisPasien like '%" & dcJenisPasien.Text & "%' AND Kelas like '%" & dcKelas.Text & "%' AND JenisItem like '" & dcJenisItem.Text & "%' AND NamaItem like '%" & dcNamaItem.Text & "%'  AND AsalPasien LIKE '%" & dcAsalPasien.Text & "%' AND KdRuangan = '" & mstrKdRuangan & "'  "

    ElseIf optApotik.Value = True Then
        frmClosingDataPelayananTM_OA_Apotik.Caption = "Closing Data Pelayanan Apotik"
...
Private Sub cmdCetak_Click()
    ...
    'frmCetakPostingDataKasirPenerimaan.Show
End Sub
...
Private Sub cmdClosingData_Click()
    ...
    Call Add_HistoryLoginActivity("Add_ClosingDataPelayanan+Loop_AddDataPelayananPasienTMOAApotikPH")
