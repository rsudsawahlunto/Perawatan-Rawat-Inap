...
lanjutICD9_:
    ...
    For i = 1 To intICD9Diplh
        If bolEditDiagnosa = True Then
        strSQL = "SELECT * FROM DetailPeriksaDiagnosa WHERE (NoPendaftaran = '" & txtNoPendaftaran.Text & "') AND (KdDiagnosa = '" & mstrKdDiagnosa & "') AND (TglPeriksa = '" & Format(dtpTglPeriksa.value, "yyyy/MM/dd HH:mm:ss") & "')  AND (KdDiagnosaTindakan = '" & Right(strKdDiagnosaTindakan(i), 5) & "')"
            Call msubRecFO(rsG, strSQL)
            If rsG.EOF = True Then
                If sp_AUDDetailPeriksaDiagnosa(dbcmd, mstrKdDiagnosa, Right(strKdDiagnosaTindakan(i), 5), "A") = False Then Exit Sub
                Else
                MsgBox "Kode Diagnosa Tindakan  " & Right(strKdDiagnosaTindakan(i), 5) & "  Sudah di inputkan ", vbInformation, "Medifirst2000 - Informasi"
            End If
        Else
...
Private Sub subLoadDiagnosaTopTen()
    strSQL = "SELECT distinct top (10) KdDiagnosa,NamaDiagnosa, SUM(JumlahPasien) AS Jumlah" _
             & " From V_DiagnosaTopTenRuanganNew where KdRuangan='" & mstrKdRuangan & "' GROUP BY NamaDiagnosa, KdDiagnosa, KdRuangan " _
