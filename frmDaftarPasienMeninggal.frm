...
Begin VB.Form frmDaftarPasienMeninggal 
   ...
   Begin VB.Frame Frame2 
      ...
      Begin VB.Label Label2 
         AutoSize        =   -1  'True
         Caption         =   "Masukkan  Nama/ No.CM/ Alamat"
         Height          =   210
         Left            =   1560
         TabIndex        =   8
         Top             =   195
...
Private Sub cmdCari_Click()
    On Error GoTo hell
    Set rs = Nothing
    strSQL = "select * from V_DaftarPasienMeninggal where([Nama Pasien] like '%" & txtParameter.Text & "%' OR Alamat like '%" & txtParameter.Text & "%' OR NoCM LIKE '%" & txtParameter.Text & "%' ) AND TglMeninggal between '" & Format(dtpAwal.Value, "yyyy/MM/dd HH:mm:00") & "' and '" & Format(dtpAkhir.Value, "yyyy/MM/dd HH:mm:59") & "' and KdRuangan='" & mstrKdRuangan & "'"
    rs.Open strSQL, dbConn, adOpenStatic, adLockOptimistic
    Set dgPasienMeninggal.DataSource = rs
    Call SetGridPasienMeninggal
    If dgPasienMeninggal.ApproxCount = 0 Then Exit Sub
    Exit Sub
hell:
    msubPesanError
End Sub
...
Private Sub cmdCetak_Click()
    ...
    cetak = "DPMeninggal"
    frmCetakDaftarPasienMeninggal.Show
    cmdCetak.Enabled = True
    Exit Sub
hell:
    msubPesanError
...
Private Sub cmdCetakSuratKeteranganMeninggal_Click()
    On Error GoTo hell
...
Private Sub Form_KeyDown(KeyCode As Integer, Shift As Integer)
    ...
    Select Case KeyCode
        ...
        Case vbKeyF1
            If strShiftKey = 2 Or strShiftKey = 3 Then
                Call cmdCetak_Click
            Else
                cmdCetak.Enabled = False
                strSQL = " select * from V_DaftarPasienMeninggal where NOCM='" & dgPasienMeninggal.Columns("NoCM").Value & "'  AND TglMeninggal between '" & Format(dtpAwal.Value, "yyyy/MM/dd HH:mm:00") & "' and '" & Format(dtpAkhir.Value, "yyyy/MM/dd HH:mm:59") & "'"
...
Private Sub Form_Load()
    ...
    frameJudul.Caption = "Daftar Pasien Meninggal di " + mstrNamaRuangan
    dtpAkhir.Value = Now
