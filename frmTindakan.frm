        ...
        With fgPelayanan
            For i = 1 To rs.RecordCount
                For j = 1 To fgPelayanan.Rows - 2
                     strKodePelayananRS = rs.Fields("KdPelayananRS")
                ...
                intRowNow = i
                .TextMatrix(intRowNow, 0) = rs.Fields("KdPelayananRS") 'strKodePelayananRS
                 strKodePelayananRS = rs.Fields("KdPelayananRS")
                .TextMatrix(intRowNow, 1) = rs.Fields("NamaPelayanan") 'txtNamaPelayanan.Text
                .TextMatrix(intRowNow, 2) = rs.Fields("JmlPelayanan") 'CInt(txtKuantitas.Text)
                
                
                 If rs.Fields("StatusCito") = "Y" Or rs.Fields("StatusCito") = "1" Then strCito = 1
                If rs.Fields("StatusCito") = "T" Or rs.Fields("StatusCito") = "T" Then strCito = 0

                ' If rs.Fields("StatusCito") = 1 Then optCito(0).Value = True Else optCito(1).Value = True
                If rs.Fields("StatusCito") = "Y" Then optCito(0).Value = True
                If rs.Fields("StatusCito") = "T" Then optCito(1).Value = True
                subcurTarifCito = sp_Take_TarifBPT
                .TextMatrix(intRowNow, 3) = IIf(rs.Fields("BiayaSatuan") = 0, 0, Format(rs.Fields("BiayaSatuan"), "#,###")) 'curBiaya
                .TextMatrix(intRowNow, 4) = Format(CCur(rs.Fields("BiayaTotal")) + subcurTarifCito, "#,###")
                .TextMatrix(intRowNow, 8) = subcurTarifCito

                .TextMatrix(intRowNow, 5) = mdTglBerlaku
                If IsNull(rs.Fields("IdDokterOrder")) Then
                    .TextMatrix(intRowNow, 6) = UserID
                Else
                    .TextMatrix(intRowNow, 6) = rs.Fields("IdDokterOrder")
                    mstrKdDokter = rs.Fields("IdDokterOrder")
                End If
...
Private Sub optCito_Click(index As Integer)
 ...
 Else
    ...
    Call msubRecFO(rs, strSQL)
     ...
     If index = 0 Then
        strCito = "1"
        fgPelayanan.TextMatrix(intRowNow, 7) = "Y"
        
        subcurTarifCito = sp_Take_TarifBPT
         
        fgPelayanan.TextMatrix(intRowNow, 3) = IIf(rs.Fields("BiayaSatuan") = 0, 0, Format(rs.Fields("BiayaSatuan"), "#,###")) 'curBiaya
        fgPelayanan.TextMatrix(intRowNow, 4) = Format(CCur(rs.Fields("BiayaTotal")) + subcurTarifCito, "#,###")
    ...
    Else
        strCito = "0"
        fgPelayanan.TextMatrix(intRowNow, 7) = "T"
        subcurTarifCito = sp_Take_TarifBPT
        fgPelayanan.TextMatrix(intRowNow, 8) = 0
        fgPelayanan.TextMatrix(intRowNow, 4) = IIf(rs.Fields("BiayaSatuan") = 0, 0, Format(rs.Fields("BiayaSatuan"), "#,###")) + 0
...
Private Sub txtNamaPerawat_Change()
    On Error GoTo errLoad
    Call subLoadListPemeriksa("where [Nama Pemeriksa] LIKE '%" & txtNamaPerawat.Text & "%' /*AND KdRuangan = '" & mstrKdRuangan & "'*/")
    lvPemeriksa.Visible = True
