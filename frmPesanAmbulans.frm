...
'    If dgKategoryInfra.ApproxCount = 0 Then Exit Sub
    vLaporan = ""
...
On Error GoTo errLoad
    ...
    With dbcmd
        ...
        .Parameters.Append .CreateParameter("NoCM", adChar, adParamInput, 6, IIf(txtNoCM.Text = "", Null, txtNoCM.Text))
...
'    strSQL = "SELECT     TOP (200) IdPegawai, NamaLengkap FROM         DataPegawai " '"
'    Call msubDcSource(dcPenanggunjawab, rs, strSQL)
'    Call msubDcSource(dcPegawaiVerifikator, rs, strSQL)
...
'    strSQL = " SELECT dbo.Pasien.NoCM, dbo.Pasien.NamaLengkap, dbo.PemakaianKamar.NoPakai, dbo.PemakaianKamar.NoPendaftaran " & _
'             " FROM dbo.Pasien INNER JOIN " & _
'             " dbo.PemakaianKamar ON dbo.Pasien.NoCM = dbo.PemakaianKamar.NoCM " & _
'             " where dbo.Pasien.NamaLengkap like '%" & txtNamaPasien.Text & "%' "
    
    strSQL = "select NoPendaftaran,NoCM,[Nama Pasien] from V_DaftarPasienRIAktif where [Nama Pasien] like '%" & txtNamaPasien.Text & "%' and Ruangan='" & strNNamaRuangan & "'"
...
'    End If
'
'    If dgPasien.visible = True And a = False Then
'        dgPasien.visible = True
'    Else
'        dgPasien.visible = False
'    End If
''    dgPasien.Visible = False
...
Private Sub dgPasien_GotFocus()
'    a = True
End Sub

'Private Sub dgPasien_RowColChange(LastRow As Variant, ByVal LastCol As Integer)
'    On Error GoTo hell
'    With dgPasien
'        If .ApproxCount = 0 Then Exit Sub
''        tempStatusTampil = True
'        txtNamaPasien.Text = .Columns("NamaLengkap")
'        txtNoPakai.Text = .Columns("NoPakai")
'        txtNoCM.Text = .Columns("NoCM")
'        txtNmPnggungJwbPsien.SetFocus
''        tempStatusTampil = False
'        .visible = False
'    End With
